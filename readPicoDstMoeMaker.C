...
Double_t minJetPt = 10.0;
Double_t minSignalJetPt = 10.0;

double fJetRadius = 0.4;  // 0.4, 0.3, 0.2
double fJetConstituentCut = 1.0; // correlation analysis: 2.0, jet shape analysis: 2.0 (been using 2.0 for corrections)
bool runSubtractor = kFALSE;   // kFALSE -> off

void readPicoDstMoeMaker(const Char_t *inputFile="Run14_P18ih_HPSS_15164046.list", const Char_t *outputFile="Feb18Test.root", Int_t nEv = 10, const Char_t *fOutJobappend="")
{
        Int_t nEvents = 1000;
        if(nEv > 100){
          nEvents = 100000000;
        ...
        if((RunYear == mRun12) && doTEST) inputFile = "ppTest2020.list";
        if((RunYear == mRun14) && doTEST) inputFile = "Run14_P18ih_HPSS_15164046.list"; //"Run_15164046_files.list"; // "Run_15151042_files.list";
        ...
        //for quick testing:
        //inputFile = "pp_test.list";
        //inputFile = "Run14_short.list";
        ...
        bool doTrkEff = kFALSE; //tracking efficiency switch
        ...
        Int_t effType = StJetFrameworkPicoBase::kNormalPtEtaBased; // options: kNormalPtEtaBased, kPtBased, kEtaBased, kHeaderArray

        // systematic flags
        Int_t kSystematicUncType = kDoNothing;  // kDoNothing, kTrkEffMin, kTrkEffMax - avail options 

        // jet type flags
        Int_t fJetType;
        if(RunYear == mRun12) fJetType = kChargedJet;
        if(RunYear == mRun14) fJetType = kChargedJet; // kFullJet
  ...
  //StPicoDstMaker *picoMaker = new StPicoDstMaker(0,inputFile,"picoDst"); // writing Pico's
        ...
        jetTask->SetMinJetPt(minJetPt);             // 10.0 signal jets
        ...
        jetTask->SetdoConstituentSubtr(runSubtractor);  // implement constituent subtractor, if TRUE, don't want to subtract underlying event Rho
        jetTask->SetRejectBadRuns(RejectBadRuns);// switch to load and than omit bad runs
        //jetTask->SetDoEffCorr(doTrkEff); //switch for tracking efficiency
        ...
        if(doBackgroundJets) {
          ...
          jetTaskBG->SetMinJetPt(minJetPt);
          ...
          jetTaskBG->SetdoConstituentSubtr(kFALSE);    // don't want to subtract background from backgrund :)
          jetTaskBG->SetRejectBadRuns(RejectBadRuns);            // switch to load and than omit bad runs
          //jetTaskBG->SetDoEffCorr(doTrkEff); //tracking efficiency
        ...
        //rhoTask->SetDoEffCorr(doTrkEff); //tracking efficiency
        ...
        StJetShapeMTPMaker *anaMaker = new StJetShapeMTPMaker("MoeMaker", picoMaker, outputFile, "JetMaker", "StRho_JetsBG");
        ...
        anaMaker->SetMinJetPt(minSignalJetPt);                     // minimum jet pt cut
        anaMaker->SetJetMaxTrackPt(30.0);                 // jet track bias
        anaMaker->SetJetMaxTowerEt(30.0);                  // jet tower bias
        anaMaker->SetMinTrackPt(fJetConstituentCut);                    // track quality cut (not related to constituents!)
        ...
        anaMaker->SetTrackEtaRange(-1.0 + fJetRadius, 1.0 - fJetRadius);           // track eta acceptance
        ...
        anaMaker->SetDoEffCorr(doTrkEff);                      // track reco efficiency switch
        //anaMaker->SetTrackEfficiencyType(effType);             // tracking efficiency type:  pt-eta, pt-based, eta-based
        //anaMaker->SetSystematicUncType(kSystematicUncType);    // systematic uncertainty type to run/toggle
